...
import (
	...
	"github.com/ogzhanolguncu/distributed-counter/part3/crdt"
...
// Full sync with all peers, not just a random subset
func (n *Node) fullSync() {
	peers := n.peers.GetPeers()
	if len(peers) == 0 {
		n.logger.Info("no peers available for full sync")
		return
	}

	n.logger.Info("performing full sync with all peers", "peer_count", len(peers))

	ctx, cancel := context.WithTimeout(n.ctx, n.config.FullSyncInterval/2)
	defer cancel()
	...
	for _, peer := range peers {
		peerAddr := peer // Shadow the variable for goroutine
		g.Go(func() error {
			n.logger.Info("sending full state",
				"to", peerAddr,
				"counter", n.counter.Value())
	...
	if err := g.Wait(); err != nil {
		n.logger.Error("full sync round failed", "error", err)
	}
}
